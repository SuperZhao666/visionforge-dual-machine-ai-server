...
public final class DualMachineReleaseSecurityConfig {
    ...
    private static final int MAXIMUM_PAIR_CREDENTIAL_KEYS = 3;
    ...
    public static final class Material {
        ...
        public final PairGenerationCredentialV1Verifier
        ...
        private Material(
            ...
            this.pairGenerationCredentialVerifier =
                    pairGenerationCredentialVerifier;
        }
    }
    ...
    public static Material load(Context context)
        ...
        List<PublicKey> pairCredentialKeys = parsePairCredentialKeys(
    ...
    static List<PublicKey> parsePairCredentialKeys(String encodedFiles)
            throws GeneralSecurityException {
        if (encodedFiles == null || encodedFiles.isBlank()) {
            throw new GeneralSecurityException(
                    "dual_machine_pair_credential_keys_missing");
        }
        String[] values = encodedFiles.split(";", -1);
        if (values.length < 1
                || values.length > MAXIMUM_PAIR_CREDENTIAL_KEYS) {
            throw new GeneralSecurityException(
                    "dual_machine_pair_credential_keyring_size_invalid");
        }
        List<PublicKey> result = new ArrayList<>(values.length);
        Set<String> keyFingerprints = new LinkedHashSet<>();
        for (String encodedFile : values) {
            if (encodedFile.isBlank()) {
                throw new GeneralSecurityException(
                        "dual_machine_pair_credential_key_empty");
            }
            final byte[] pemBytes;
            try {
                pemBytes = Base64.getDecoder().decode(encodedFile);
                ...
                throw new GeneralSecurityException(
                        "dual_machine_pair_credential_key_wrapper_invalid",
                        invalid);
            }
            PublicKey publicKey = parsePairCredentialPublicPem(pemBytes);
            String fingerprint = sha256Hex(publicKey.getEncoded());
            if (!keyFingerprints.add(fingerprint)) {
                throw new GeneralSecurityException(
    ...
    private static PublicKey parsePairCredentialPublicPem(byte[] pemBytes)
            throws GeneralSecurityException {
        if (pemBytes.length < 256 || pemBytes.length > 16 * 1024) {
            throw new GeneralSecurityException(
                    "dual_machine_pair_credential_public_pem_size_invalid");
        }
        String pem = new String(
                pemBytes, StandardCharsets.US_ASCII).trim();
        if (!pem.startsWith(PUBLIC_KEY_BEGIN)
                ...
                || pem.indexOf(PUBLIC_KEY_BEGIN, 1) >= 0) {
            throw new GeneralSecurityException(
                    "dual_machine_pair_credential_public_pem_invalid");
        }
        String body = pem.substring(
                PUBLIC_KEY_BEGIN.length(),
                pem.length() - PUBLIC_KEY_END.length())
        ...
        try {
            ...
            throw new GeneralSecurityException(
                    "dual_machine_pair_credential_public_pem_base64_invalid",
                    invalid);
        }
        PublicKey key = KeyFactory.getInstance("RSA").generatePublic(
                new X509EncodedKeySpec(der));
        if (!(key instanceof RSAPublicKey)) {
            throw new GeneralSecurityException(
                    "dual_machine_pair_credential_public_key_not_rsa");
        }
        RSAPublicKey rsaPublicKey = (RSAPublicKey) key;
        if (rsaPublicKey.getModulus().bitLength() < 3072) {
            throw new GeneralSecurityException(
        ...
        if (!MessageDigest.isEqual(key.getEncoded(), der)) {
            throw new GeneralSecurityException(
                    "dual_machine_pair_credential_public_key_not_canonical");
        }
        return key;
    }
